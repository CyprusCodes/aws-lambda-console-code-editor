...
if (fs.existsSync(patchesDir)) {
  ...
  patches.forEach((patch) => {
    ...
    try {
      // First, check the patch to see if it will apply cleanly
      exec(`git apply --check ${patchPath}`, { stdio: "inherit" });

      exec(`git apply ${patchPath}`, { stdio: "inherit" });
      ok(`Patch applied: ${patch}`);
    } catch (e) {
      error(`Error applying patch: ${patch}`);
      process.exit(1);
    }
  });
} else {
  note("No patches directory found. Skipping patch application.");
}
